...
(function() {
    ...
    $R.opts = {
        ...
        // buttons
        buttons: ['html', 'format', 'bold', 'italic', 'fontcolor', 'deleted', 'lists', 'webuploadImage', 'webuploadVideo', 'link'],
    ...
    $R.add('class', 'cleaner.figure', {
        ...
        convert: function(html, rules)
        {
            ...
            $wrapper.find('video').each(this._convertImage.bind(this));
            ...
            // widgets
            var name = ''
            if(html.match('<figure><video')) {
                name = 'image'
            }
            $wrapper.find('figure').not('.redactor-component, .redactor-figure-code').each(this._convertWidget.bind(this, name));
            // contenteditable
        ...
        _convertWidget: function(name, node)
        {
            var name = name || ''
            if (this._isNonEditable(node)) return;
            ...
            $node.attr('data-redactor-type', name ? name : 'widget');
    ...
    $R.add('service', 'insertion', {
        ...
        insertHtml: function(html, clean)
        {
            ...
            var fragment, except;
            // var ele = this.element.getElement();
            // var val = ele.val();
            // ele.val(val.replace(/<p><\/p>$/g, ''));
    ...
    $R.add('module', 'source', {
        ...
        show: function()
        {
            ...
            var html = $source.val();
            html = html.replace(/<figure\s.+><video/g, '<figure><video');
    ...
    $R.add('module', 'contextbar', {
        ...
        set: function(e, node, buttons, position)
        {
            ...
            // 应急  空白区域点击出现弹窗问题
            if(e.target.nodeName == 'A' || e.target.nodeName == 'VIDEO' || e.target.nodeName == 'IMG') {
    ...
    $R.add('module', 'image', {
        ...
        ondropimage: function(e, files, clipboard,parse,ee)
        {
            var _this;
            ...
            if(parse=="parse"){
                ...
                reader.onloadend = function (e) {
                    ...
                    //if (!_this.opts.imageUpload) return;
                    var options = {
                        url: url,
                    ...
                    _this.app.api('module.upload.send', options);
                }
            }else{
                ...
                for(var j=0;j<files.length;j++){
                    readers[j] = new FileReader();
                    readers[j].readAsDataURL(files[j]);
                    readers[j].onloadend = function (e) {
                        filedata = e.target.result;
                        //if (!_this.opts.imageUpload) return;
                        var options = {
                            url: url,
        ...
        _buildPreview: function()
        {
            ...
            // 添加视频编辑
            if(this.$image.$element && this.$image.$element.nodes[0] && this.$image.$element.nodes[0].tagName === 'VIDEO') {
                $previewImg = $R.dom('<video>');
            } else {
                $previewImg = $R.dom('<img>');
            }
    ...
    $R.add('class', 'image.component', {
        ...
        _init: function(el)
        {
            ...
            {
                this.parse($figure);
                this.$element = this.find('img').length ? this.find('img') : this.find('video');
            }

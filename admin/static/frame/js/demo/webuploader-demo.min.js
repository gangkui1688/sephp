jQuery(function () {
        ...
        if (e !== k) {
            switch (c.removeClass("state-" + k), c.addClass("state-" + e), k = e) {
                ...
                case"paused":
                    ...
                    break;
                case"confirm":
                    if (f.hide(), c.text("开始上传").addClass("disabled"), a = n.getStats(), a.successNum && !a.uploadFailNum){
    ...
    n = WebUploader.create({
        ...
        swf: "static/frame/js/plugins/webuploader/Uploader.swf",
        disableGlobalDnd: !0,//是否禁掉整个页面的拖拽功能
        chunked: true,
        chunkSize: 1048576, //分片，每片1M
        chunkRetry: 3,      //如果某个分片由于网络问题出错，允许自动重传多少次
        threads: 2,         //上传并发数。允许同时最大上传进程数。
        server: "?ct=public&ac=layer_add_file",
        fileNumLimit: 300,//验证文件总数量, 超出则不允许加入队列
        fileSizeLimit: false,//验证文件总大小是否超出限制, 超出则不允许加入队列
        fileSingleSizeLimit: 104857600000,//验证单个文件大小是否超出限制, 超出则不允许加入队列。

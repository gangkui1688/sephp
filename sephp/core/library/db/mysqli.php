...
//
    ...
    public static function init_db($name = null, $config_file = null, $default_instance = false)
    {
        ...
        self::$config[$name] = config::get('mysql');
    ...
    {
        foreach(static::$_instance as $_name => $_handler)
        {
            if (is_object($_handler) && (!$name || $name == $_name))
            {
                @mysqli_close($_handler);
                // unset(static::$_instance[$_name]->_handler);
    ...
    public function quote_value($fields)
    {
        ...
        {
            throw new \Exception("The field value is not string , {$field} : " . var_export($value, 1), -1);
        }

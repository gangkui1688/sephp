...
class sys_verifiy
{
    ...
    {
        // 图片宽(px)
        $this->config['imageW'] = empty($this->config['imageW']) ? $this->config['length'] * $this->config['fontSize'] * 1.5 + $this->config['length'] * $this->config['fontSize'] / 2 : $this->config['imageW'];
        // 图片高(px)
        $this->config['imageH'] = empty($this->config['imageH']) ? $this->config['imageH'] = $this->config['fontSize'] * 2.5 : $this->config['imageH'];
        ...
        for ($i = 0; $i < $this->config['length']; $i++) {
            ...
            imagettftext($this->_image, $this->config['fontSize'], mt_rand(-40, 40), $codeNX, $this->config['fontSize'] * 1.6, $this->_color,$ttfPath, $code[$i]);
        }

...
class pub_mod_goods extends pub_mod_model
{
    public static
        ...
        $_pk           = 'goods_id',
        $_fields       = [
                'goods_id'      => ['type' => 'int','required' => true, 'comment' => '商品ID'], //订单号
                'goods_sn'      => ['type' => 'text', 'required' => true, 'comment' => '商品编号'], //服务id
                ...
                'comments_count'   => ['type' => 'int', 'default' => 0, 'comment' => '评论次数'], //申请金额
                'view_count'       => ['type' => 'int', 'default' => 0, 'comment' => '浏览次数'], //申请金额
                'buy_count'        => ['type' => 'int', 'default' => 0, 'comment' => '购买次数'], //申请金额
    ...
    public static function data_format($data = [])
    {
        ...
        if(isset($data['image_default_id']))
        {
            ...
            //array_walk($data['image_default_id'], function(&$v){ $v = sephp::$_config['upload']['filelink'].'/'.$v; });
        }
        //print_r($data);exit;
        return $data;
    }
